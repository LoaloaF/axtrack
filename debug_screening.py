...
from config import DEPLOYED_MODEL_DIR, DEFAULT_DEVICE, DEFAULT_NUM_WORKERS, RAW_INFERENCE_DATA_DIR

from utils import turn_tex
from utils import get_data_standardization_scaler
...
def get_params(num_workers=None, device=None):
...
def get_model(parameters, print_params=True):
...
def setup_inference(dest_dir, num_workers=DEFAULT_NUM_WORKERS, device=DEFAULT_DEVICE):
    parameters = get_params(num_workers, device)
...
def prepare_input_data(imseq_fname, mask_fname, metadata_fname, parameters, 
    ...
    cache_dir = None if not use_cached_datasets else RAW_INFERENCE_DATA_DIR
    timelapse = UnlabelledTimelapse(RAW_INFERENCE_DATA_DIR, imseq_fname, 
                                    mask_fname, metadata_fname, parameters, 
                                    standardize=stnd_scaler, from_cache=cache_dir, 
                                    cache=RAW_INFERENCE_DATA_DIR)
    timelapse.structure_outputchannel_coo = (0,3500)
    if check_preproc:
        preproc_file = save_preproc_metrics(dest_dir, timelapse, train_data)
