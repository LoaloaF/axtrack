...
def get_astar_path(source, target):
...
for axon in which_axons:
    ...
    coo_path = get_astar_path((anchor_y[0], anchor_x[0]), (anchor_y[-1],anchor_x[-1]))
    sparse.save_npz(f'{outputdir}/axon_reconstructions/{axon}.npz', coo_path)
    
    distances = []
    for t in range(len(timepoints)):
        print(f't:{t}...', end='')
        coo_path = get_astar_path((anchor_y[t], anchor_x[t]), target)
